...
public interface StudentRepository extends JpaRepository<Student, Long> {
  ...
  */
  ...
  // This is also Query method, we have to take care while writing the name of the method, camel
  // case and variable name.
  ...
  // @Query("from Student s where s.emailId = ?1") select is optional, and we can replace the above
  // query with this
  ...
  *select first_name from tbl_student where guardian_name = ?1 */
  ...
  /*  We don't need @Param if the method parameter name and the Query arguments have the same name.
    @Query("Select u from Student u where u.studentId = :id and u.firstName= :name")
    List<Student> getStudentsByStudentIdAndFirstName(Long id, String name);
  */

  //When we are using native query, we have to use the column name, compared to entity or field name while using JPQL
  @Query(
  ...
  /* Whenever we are doing some modification(update/delete/insert). to the DB using a query, we have to use @Modifying annotation and for modification
  ...
  @Query(
      ...
      nativeQuery = true)
/*  OR
  @Query("UPDATE Student s SET s.firstName= :name WHERE s.emailId=:mail")*/
  @Modifying
  @Transactional
  int updateStudentByEmailId(String name, @Param("mail") String email);
  ...
  @Query(
      value =
          "select first_name as firstName, last_name as lastName from tbl_student s where s.first_name=?1",
      nativeQuery = true)
