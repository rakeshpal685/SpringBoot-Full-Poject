...
public class EmployeeController {
    ...
    //Here I am using ResponseEntity as the return type because it can give a lot of info back when this
    //url is hit, like header, code etc.
    @PostMapping()
    public ResponseEntity<Employees> saveEmployee(@RequestBody Employees employees) {
        return new ResponseEntity<Employees>(employeeService.saveEmployees(employees), HttpStatus.CREATED);
    }
    ...
    @PutMapping("{id}")
    public ResponseEntity<Employees> updateEmployee(@PathVariable("id") int empid, @RequestBody Employees employee) {
        return new ResponseEntity<Employees>(employeeService.updateEmployee(employee, empid), HttpStatus.OK);
    }
